...
// 업로드 옵션 선택 모달 표시
function showUploadOptions(type) {
    console.log('업로드 옵션 표시:', type);
    
    // 기존 모달이 있으면 제거
    const existingModal = document.getElementById('upload-options-modal');
    if (existingModal) {
        ...
        <div class="upload-options-overlay">
            <div class="upload-options-content">
                <div class="upload-options-header">
                    <h3>사진 업로드 방법 선택</h3>
                    <button class="upload-close-btn" onclick="closeUploadOptions()">&times;</button>
                </div>
                <div class="upload-options-buttons">
                    <button class="upload-option-btn gallery-btn" onclick="selectGallery('${type}')">
                        <i class="fas fa-images"></i>
                        <span>갤러리에서 선택</span>
                        <small>기존 사진을 선택합니다</small>
                    </button>
                    <button class="upload-option-btn camera-btn" onclick="selectCamera('${type}')">
                        <i class="fas fa-camera"></i>
                        <span>카메라로 촬영</span>
                        <small>지금 바로 사진을 촬영합니다</small>
                    </button>
                </div>
            </div>
        </div>
    ...
    // 모달 스타일 추가
    if (!document.getElementById('upload-options-styles')) {
        const styles = document.createElement('style');
        styles.id = 'upload-options-styles';
        styles.textContent = `
...
function selectGallery(type) {
    console.log('갤러리 선택:', type);
    closeUploadOptions();
    const input = document.getElementById(`photo-input-${type}`);
    if (input) {
        input.click();
    }
}

// 카메라 선택 (MediaDevices API 사용)
async function selectCamera(type) {
    console.log('카메라 선택:', type);
    closeUploadOptions();
    
    // 모바일에서는 직접 input[type="file"]의 capture 속성을 사용
    // 이렇게 하면 권한 모달 없이 카메라 앱이 바로 열림
    const input = document.getElementById(`camera-input-${type}`);
    if (input) {
        // 카메라 설정을 위한 추가 속성 설정
        input.setAttribute('capture', 'user'); // 전면카메라(셀프카메라)
        input.setAttribute('data-flash', 'off'); // 플래시 비활성화
